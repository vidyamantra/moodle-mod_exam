...
if (($exam->attempts == 0) || $attemptedattempt < $exam->attempts) {
    echo $OUTPUT->single_button(new moodle_url('/mod/exam/attempt.php?id='.$id,
                                               array('id' => $id)), get_string('attemptexamnow', 'exam'));
    if ($attemptedattempt > 0) {
        echo $OUTPUT->heading(quiz_get_grading_option_name($exam->grademethod) .' grade : '. number_format($grade[$USER->id]->rawgrade,
                                                                                                           2). ' / ' .number_format($exam->grade, 2));
    }
} else {
    echo $OUTPUT->heading(get_string('yourfinalgradeis', 'exam',
                                     number_format($grade[$USER->id]->rawgrade, 2)). number_format($exam->grade, 2));
